...
(int, byte[,]) parseBoards() {
	int nBoards = (lines.Length - 1) / 6;
	var rslt = new byte[nBoards,25];
...
void printBoard(byte[,] boards, int[] bits, int board) {
	for(int i = 0; i < 5; i++) {
		for(int j = 0; j < 5; j++) {
			int cell = 5*i+j;
...
int boardScore(byte[,] boards, int[] bits, int winner, int n)
{
	int sum = 0;
...
var bWon = new bool[nBoards];

int n = -1;
int winner = -1;
int wonBoards = 0;
while(wonBoards != nBoards) {
	n++;
	for (int board = 0; board < nBoards; board++) {
		...
		if(!bWon[board] && checkWin(bits, board)) {
			winner = board;
			wonBoards++;
			bWon[board] = true;
		}
