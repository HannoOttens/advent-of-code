...
(int vers, int type, ulong rslt) readPacket(ref StrSpan s) {
	int vers = readVersion(ref s);
	int type = readType(ref s);
	ulong rslt = 0U;
	if (type == 4) {
		// Literals
		rslt = readLiteral(ref s);
	} else {
		// Operators
		...
		if (lngT == 0) {
			...
			int strt = s.Length;
			(int v1, int t1, rslt) = readPacket(ref s);
			while(strt - s.Length < lgth) {
				(int v2, int t2, ulong r2) = readPacket(ref s);
				rslt = operate(type, rslt, r2);
			}
		} else {
			// Number of packets
			int subPacks = subPackets(ref s) - 1;
			(int v1, int t1, rslt) = readPacket(ref s);
			while(subPacks-- > 0) {
				(int v2, int t2, ulong r2) = readPacket(ref s);
				rslt = operate(type, rslt, r2);
...
ulong operate(int type, ulong r1, ulong r2) {
	switch(type) {
		case 0: return r1 + r2;
		case 1: return r1 * r2;
		case 2: return Min(r1,r2);
		case 3: return Max(r1,r2);
		case 5: return (r1 >  r2 ? 1U : 0U);
		case 6: return (r1 <  r2 ? 1U : 0U);
		case 7: return (r1 == r2 ? 1U : 0U);
	}
...
ulong readLiteral(ref StrSpan s) {
	int indx = 0;
	...
	return Convert.ToUInt64(nmbr.ToString(), 2);
}
