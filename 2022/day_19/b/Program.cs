...
int openGeodes(Blueprint bp, Vec invn, Vec robo, int time = 32) {
	maxGeo = Max(maxGeo, invn.geo);

	// Base conditions
	if (invn.geo + time*robo.geo + gauss(time) < maxGeo) return 0;
...
foreach (var bp in blueprints) {
	...
	value *= openGeodes(bp, new(0,0,0,0), new(1,0,0,0));
}
