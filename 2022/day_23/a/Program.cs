...
elfMap = new(elves);
Console.WriteLine($"Elves after: {elves.Count}");

// Too high: 4345
// Answer for someone else: 4000
// 4179? Nope..
// Make bounding box
draw();
(Posi minP, Posi maxP) = boundingBox();
Console.WriteLine($"Min: {minP}");
Console.WriteLine($"Max: {maxP}");
Posi size = maxP - minP;
Console.WriteLine((Abs(1+size.PosX))*(Abs(1+size.PosY))-elves.Count);
Console.WriteLine(82*85-elves.Count);
