...
int[] buisiness = new int[monkeys.Length];

for (int round = 0; round < 20 * monkeys.Length; round++) {
	int indx = round % monkeys.Length;
	var monkey = monkeys[indx];
	buisiness[indx] += monkey.items.Count;
	while (monkey.items.Count > 0) {
		ulong newI = monkey.op(monkey.items.Dequeue()) / 3;
...
foreach(var i in buisiness) Console.WriteLine(i);
...
// Dit werkt niet en ik weet niet waarom: Dan maar simpel
// string getField (string srce, string field) 
//	=> srce.Substring(srce.IndexOf(field) + field.Length
// 	                , srce.IndexOf("\r\n", srce.IndexOf(field));
